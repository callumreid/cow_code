...
| + | No voice input (Claude/ChatGPT parity) | **Voice dictation**: mic button in the composer (both layouts) records via MediaRecorder, the engine's new `POST /transcribe` sends it to OpenAI `gpt-4o-transcribe` with the connected key (`OPENAI_API_KEY` fallback), and the text is inserted at the caret like typed input so file pills survive. Red pulse while recording, mic force-stopped on unmount, Electron `media` permission + macOS `NSMicrophoneUsageDescription` wired for packaged builds. |
| + | Search only matched titles | **Full-text search across conversation content**: the search param now also matches visible message text (bounded LIKE join over text/reasoning parts; synthetic/ignored/tool payloads excluded; early-terminates on hits — no migration). Home search shows instant title matches plus a debounced "Messages" section, deduped, one keyboard-nav list. FTS5 is the planned v2 if very large DBs report slow misses. |
| + | Only running work was visible | Activity panel retains **finished** tools/subagents (status, duration, per-subagent tokens/cost snapshot), Clear button, `/activity` command, chip stays reachable at idle. |
| + | No persistent agent memory | `experimental.memory` (on for desktop/cli): a path-jailed memory tool over `~/.config/opencode/memory` + project `.opencode/memory`, with each scope's `MEMORY.md` index injected at session start under a verify-before-use preamble. Bypass-permissions mode (global, loud indicator, deny rules still win) and a project-wide Usage view (`mod+shift+u`) round out the harness parity set. |
| 🐄 | Cow theme | The app icon is a cow (full icns/ico/png ladder, all channels); the packaged app, dock, window title, menus, About panel, and every visible string read "cow_code" (the `opencode://` scheme, bundle ids, storage keys, and the third-party OpenCode Zen provider are deliberately untouched); the brand mark/wordmark/splash are a cow face. **Pasture** theme in the picker: meadow green, cream, soft brown, light + dark. Settings → Sounds → "Moo on completion" (off by default): a fully synthesized Web Audio moo when a turn completes while the window is unfocused. ~1 in 20 sessions shimmer "Grazing" instead of "Thinking". Moo. |

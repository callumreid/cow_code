...
| 8 | Meaningfully slow vs other harnesses | Investigated + ranked (see `perf` notes): git snapshots on the stream tap, LSP diagnostics awaited inside edit tools, sqlite write amplification, O(n²) markdown re-lex. Fixes deliberately deferred until profiled against the Electron sidecar (Node, not Bun). |

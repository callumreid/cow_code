...
# behind (GitHub refuses update-branch for queued PRs, which just logs). A conflict is reported by
# DM, never resolved automatically.
...
while IFS= read -r line; do
  ...
  case "$status" in
    ...
    BEHIND)
      ...
      if printf '%s' "$out" | grep -q -i 'conflict'; then conflicts+=("coval-ai/$repo#$number"); echo "$(date -Iseconds): conflict on $repo#$number" >> "$LOG"
...
if [ "${#conflicts[@]}" -gt 0 ]; then
  "$HOME/bin/cow-notify.sh" "keep-updated: these PRs conflict with their base and need a manual rebase: ${conflicts[*]}" >> "$LOG" 2>&1
fi

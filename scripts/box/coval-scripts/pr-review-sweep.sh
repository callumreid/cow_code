...
# pinned to GLM 5.2 on the plain Alibaba provider. Fires at :09 and :39,
...
3. /Users/bronson/.agents/skills/review-queue/SKILL.md and /Users/bronson/.agents/skills/review-pr/SKILL.md — every rule applies except the two the sweep SKILL.md explicitly overrides.
...
Then run the sweep per the SKILL.md: build the queue from EVERY source that SKILL.md lists. It is the single source of truth for how many sources there are and what each covers — enumerate them from it, never from memory and never from this prompt, which has drifted behind it twice. Three things in it are load-bearing. (a) Source 5, added 2026-09-09: PRs callumreid has ALREADY reviewed whose head has moved since — classification NEW-WORK, discovered because find-queue.sh now unions --reviewed-by with --review-requested. Submitting a review makes GitHub clear you from requested_reviewers, so before this a PR left the queue permanently the moment it was reviewed and no re-review could ever be discovered. Those rows carry request-kind '-' precisely BECAUSE the review consumed the request, so do NOT drop them when you apply Source 1's direct-only filter — that is the exact bug Source 5 exists to close. (b) jakelevi is excluded from the un-tagged sources: review one of his PRs only when callumreid is currently in requested_reviewers. (c) Drafts are excluded from EVERY source — Dana's included; her earlier draft carve-out is retired (2026-09-10), and post-inline-review.py refuses APPROVE or REQUEST_CHANGES on a draft, so never post a review event on one. Dedupe by repo#number, and act only on genuinely new heads. Skip any PR with an approval standing at its current head, whatever the approval's source — compare commit_id, never timestamps, and re-check immediately before every post. Dispatch read-only review agents (cap ~6 concurrent, group stacked PRs into one agent), and post per the verdict policy: lean APPROVE, REQUEST_CHANGES only for a confirmed P0/P1 read in the code with a concrete failure scenario, and cross-PR dependency/stack/cycle/merge-order findings are advisory-only — never sufficient for REQUEST_CHANGES. Always run post-inline-review.py with --dry-run first; after any posting error, read the review list back before retrying. Append one TSV row per PR acted on to /Users/bronson/.claude/scheduled-tasks/pr-review-sweep/sweep-log.tsv and remove every worktree you create.

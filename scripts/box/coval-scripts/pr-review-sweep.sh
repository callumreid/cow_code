...
NO_COLOR=1 /Users/bronson/.opencode/bin/opencode run --auto --attach http://127.0.0.1:4096 \
  ...
  -m cloudflare-workers-ai/@cf/zai-org/glm-5.3-flash \
...
Then run the sweep per the SKILL.md: build the queue from its four sources (direct requests only from find-queue.sh output, every open borgesius PR including drafts, every open robyoungsf PR excluding drafts, and the engineering-request backstop: any open non-draft PR where a coval-engineering member or the coval-engineering team is requested — whether or not callumreid was tagged — that has stood at least one hour with no review at its current head; discover it with one org-wide REST search then the per-PR truth pass, since review-requested team qualifiers 422 in the search API), dedupe by repo#number, and act only on genuinely new heads. Skip any PR with an approval standing at its current head, whatever the approval's source — compare commit_id, never timestamps, and re-check immediately before every post. Dispatch read-only review agents (cap ~6 concurrent, group stacked PRs into one agent), and post per the verdict policy: lean APPROVE, REQUEST_CHANGES only for a confirmed P0/P1 read in the code with a concrete failure scenario. Always run post-inline-review.py with --dry-run first; after any posting error, read the review list back before retrying. Append one TSV row per PR acted on to /Users/bronson/.claude/scheduled-tasks/pr-review-sweep/sweep-log.tsv and remove every worktree you create.

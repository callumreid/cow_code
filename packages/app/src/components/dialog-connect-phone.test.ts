...
const door = { kind: "door" as const, url: "https://ours-composition-fellow-gamma.trycloudflare.com/?_cowcode_gate=k" }

describe("phoneUrl", () => {
  test("the barn door keeps its gate key and never gets auth_token", () => {
    const url = new URL(phoneUrl(door, base, ["/Users/bronson/coval"]))
    expect(url.origin).toBe("https://ours-composition-fellow-gamma.trycloudflare.com")
